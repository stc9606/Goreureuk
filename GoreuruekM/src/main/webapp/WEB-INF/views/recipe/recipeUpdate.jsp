		...
		<div id="fh5co-main">
			<div class="container">
				<div class="row" id="recipeMain">
					...
					<div>
						<form name="form2" action="${contextPath }/recipe/recipeupdate?${_csrf.parameterName}=${_csrf.token}" method="post" enctype="multipart/form-data">
							<c:forEach var="view" items="${view}" varStatus="vs">
							<input  id="RCC_CODE" name="RCC_CODE" value="${view.RCC_CODE }">
							</c:forEach>
								...
								<thead>
									<tr>
										<td colspan="4">
											<div class="recipeMainImg">
												...
												<input type="hidden" id="RC_CODE" name="RC_CODE" value="${view.get(0).RC_CODE }">												
												<div class="recipeTitle">
													<label>제목</label> <span><input type="text" id="RC_TITLE" name="RC_TITLE" value="${view.get(0).RC_TITLE }"></span>
												</div>
												<div class="recipeInforamtion">												
													<label>설명</label> <span><textarea class="form-control" id="RC_INFORMATION" name="RC_INFORMATION" rows="10" cols="1000">${view.get(0).RC_INFORMATION }</textarea></span>
												</div>
											</div>
										</td>
									</tr>
									<tr>
										<td colspan="4"><label>작성자</label> <input type="text"
											readonly="readonly" name="RC_MMID" value="${view.get(0).RC_MMID }"></td>
									</tr>
									<tr>
									...
									<tr>
										...
										<textarea class="recipeIngredoent" id="RC_INGREDIENT" name="RC_INGREDIENT">${view.get(0).RC_INGREDIENT }</textarea></td>
									</tr>
								</thead>
							</table>
							...
							<table class="table table-bordered">
								<tbody id="stock_tbody">
									...
									<c:forEach var="view" items="${view}" varStatus="vs">
									<tr class="trset">
										<th><input type="text" class="RCC_STEP" name="RCC_STEP" readonly="readonly" value="${view.RCC_STEP }"></th>
										<td colspan="2"><textarea class="recipeContent" name="RCC_STEPINFO"
												placeholder="설명 입력">${view.RCC_STEPINFO }</textarea></td>
										<td><div>
												<input type="file" name="file" id="imagefile" accept=".gif, .jpg, .jpeg, .png" value="${view.RCC_SOURCE}">
												<button type="button" id="stepadd${vs.index }" class="stepbtn">순서추가</button>
												<button type="button" id="stepdel${vs.index }" class="stepbtn">순서제거</button>
											</div></td>
									</tr>
									</c:forEach>
								</tbody>
								<tfoot>
									<tr>
										<td class="board_menu" colspan="4">
											<button  type="button" class="optionbtn" name="btnsub">수정</button>
